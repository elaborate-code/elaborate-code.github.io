...
<aside x-data="{ visible: false }" x-cloak x-show="visible" x-transition x-on:open-hamburger.window="visible = true"
    ...
    class="sm:hidden fixed z-10 top-0 right-0 h-screen w-[75vw] p-4 flex flex-col gap-y-8 bg-white-smoke text-bloodmyst-isle">
    ...
    <div class="flex-1 flex flex-col gap-4">
        ...
        @foreach ($page->routes as $route)
            <a href="{{ $page->route($route->url, $lang) }}"
                class="block px-4 py-2 border border-bloodmyst-isle rounded-md">
                {{ $route->name }}
            </a>
        @endforeach
